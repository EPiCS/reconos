 ...
 * rawdaudio_hybrid.c
 *
 * Modified rawdaudio.c for using in MiBenchHybrid
 *
...
/* rawdaudio Softwarethread */
void *rawdaudio_swt(void* data)
{
	...
	char *inp;		//pointer to the first position of the input array
	short *outp;	//pointer to the first position of the output array
	...
	while(1)
	{
		...
		else
		{
			...
			//adpcm_coder
			adpcm_decoder(inp, outp, samples, &state);
		}
...
{
	fprintf(stderr, 
		"MiBenchHybrid rawdaudio application\n"
		"\n"
		"USAGE:\n"
		"\tMiBench version:\n"
		"\trawdaudio\n"
		...
		"\trawdaudio_hybrid <number of software threads> <number of hardware threads>\n"
...
int main(int argc, char *argv[])
{
	...
	{		
		...
		for (i = 0; i < sw_threads; i++)
		{
			...
			pthread_create(&swt[i], &swt_attr[i], rawdaudio_swt, (void*)res);
		}
		#ifndef _MIBENCHOUTPUT
		fprintf(stderr, "\n");		
		#endif
	...
	while(1) 
	{
		...
		if( rawdaudio_hybrid )
		{
			...
			indata[0] = (uint32) abuf;
			indata[1] = (uint32) sbuf;
			indata[2] = (uint32) n*2;
			indata[2] &= 0x7FFFFFFF; 		//we want to decode adpcm so the highest bit have to be 0
			indata[3] = 0;
			...
			t_stop = gettime();
			t_decode_data += calc_timediff_ms(t_start,t_stop);
		}
		else
		{
			t_start = gettime();
	...
	fprintf(stderr,  
		...
		t_write_data,
		t_terminate_threads,
		t_generate_threads+t_read_data+t_decode_data+t_write_data+t_terminate_threads
	);
	#endif	
