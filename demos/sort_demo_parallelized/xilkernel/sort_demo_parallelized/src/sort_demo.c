...
//#define DEBUG_SORT_DEMO

#define BLOCK_WORDS 2048

#define NUM_HWTS 14
#define NUM_SWTS 0
#define NUM_BLOCKS 128 // must be power of 2 dividable by PARALLELITY!

#define PARALLELITY 4 // _MUST_ correspond to the C_NUM_ENGINES generic of the sort_demo_parallelized hardware thread
...
void *sort_thread(void* data)
{
	...
	while ( 1 ) {
		...
		if(ret == UINT_MAX){
		  ...
		  {
			bubblesort( (unsigned int*) (ret+i*sizeof(int)*BLOCK_WORDS), BLOCK_WORDS);
#ifdef DEBUG_SORT_DEMO
			INFO("Sorting %i words at address 0x%x\r\n", BLOCK_WORDS, (unsigned int*) (ret+i*sizeof(int)*BLOCK_WORDS));
#endif
		  }
		}
...
void* sort_main(void * arg)
{
	...
	unsigned int checksum_copy;
#ifdef DEBUG_SORT_DEMO
	checksum_data = generate_checksum(data,NUM_BLOCKS*BLOCK_WORDS);
	checksum_copy = generate_checksum(copy,NUM_BLOCKS*BLOCK_WORDS);
	INFO("Checksum of data: %d, checksum of copy: %d \n",checksum_data, checksum_copy);
	print_data(data, BLOCK_WORDS );
#endif
	...
	INFO("Putting %i blocks into job queue: ", NUM_BLOCKS/PARALLELITY);
	for (i = 0; i < NUM_BLOCKS/PARALLELITY; i++)
	{
		INFO(" %i",i);
		//mbox_put(&mb_start,(unsigned int)data+(i*BLOCK_WORDS*PARALLELITY));
		mbox_put(&mb_start,(unsigned int)(data+i*BLOCK_WORDS*PARALLELITY));
	}

	// Wait for results
	INFO("\nWaiting for %i acknowledgements: ", NUM_BLOCKS/PARALLELITY);
	for (i = 0; i < NUM_BLOCKS/PARALLELITY; i++)
	{
		INFO(" %i",i);
		mbox_get(&mb_stop);
	...
	checksum_copy = generate_checksum(copy,NUM_BLOCKS*BLOCK_WORDS);
	INFO("Checksum of data: %d, checksum of copy: %d \n",checksum_data, checksum_copy);
	print_data(data, NUM_BLOCKS*BLOCK_WORDS );
#endif
	...
	mergesort(data,temp,NUM_BLOCKS,BLOCK_WORDS);
	data = temp;
	...
	checksum_copy = generate_checksum(copy,NUM_BLOCKS*BLOCK_WORDS);
	INFO("Checksum of data: %d, checksum of copy: %d \n",checksum_data, checksum_copy);
	INFO("Quick check sorted data ... ");

	unsigned int quick_check_result = quick_check_data(data,NUM_BLOCKS*BLOCK_WORDS,checksum_copy);
	if( quick_check_result != 0){
