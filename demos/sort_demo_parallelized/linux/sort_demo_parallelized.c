...
#define PARALLELITY 4 // _MUST_ correspond to the C_NUM_ENGINES generic of the sort_demo_parallelized hardwar thread
...
void *sort_thread(void* data)
{
	...
	else
	{
	  ...
	  {
		bubblesort( (unsigned int*) (ret+i*BLOCK_SIZE), N);
	  }
	}
...
{
  printf(
"ReconOS v3 sort demo parallelized application.\n"
...
"<num_of_blocks> must be a multiple of %i!\n",
...
int main(int argc, char ** argv)
{
	...
	printf("Putting %i blocks into job queue: ", TO_BLOCKS(buffer_size)/PARALLELITY);
	fflush(stdout);

	reconos_cache_flush();

	for (i=0; i < TO_BLOCKS(buffer_size)/PARALLELITY; i++)
	{
	  printf(" %i",i); fflush(stdout);
	  mbox_put(&mb_start,(unsigned int)data+(i*BLOCK_SIZE*PARALLELITY));
	}
	printf("\n");

	// Wait for results
	printf("Waiting for %i acknowledgements: ", TO_BLOCKS(buffer_size)/PARALLELITY);
	fflush(stdout);
	for (i=0; i < TO_BLOCKS(buffer_size)/PARALLELITY ; i++)
	{
	  printf(" %i",i);fflush(stdout);
	...
	printf("Sending terminate message to %i threads: %x", running_threads, UINT_MAX);
	fflush(stdout);
	...
	printf("Waiting for termination of %i hw and %i sw threads...\n", hw_threads, sw_threads);
	for (i=0; i<hw_threads; i++)
	{
	  printf("%i\n", i);
