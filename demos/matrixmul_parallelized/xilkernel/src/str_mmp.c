...
int *str_matrix_combine(MATRIXES **std_mmp_matrixes, int i_matrix_size) {
	int o_matrix_size = i_matrix_size;
	MATRIXES *str_mmp_matrixes;

	while(o_matrix_size < STR_MMP_INPUT_MATRIX_SIZE) {
		str_mmp_matrixes = NULL;

		while(NULL != *std_mmp_matrixes) {
			o_matrix_size = str_matrix_combine_step(&str_mmp_matrixes, std_mmp_matrixes, i_matrix_size);
		}

		i_matrix_size = o_matrix_size;
