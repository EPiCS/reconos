			  ...
			  int count, int *eof, void *data)
{
	...
	for (i = 0; i <= max; ++i) {
		...
		len += sprintf(page + len, "] %s %s %llu %llu %llu %llu [",
...
int fblock_userctl_set_flags(struct lananlmsg *lmsg, unsigned int flags)
{
	struct fblock *fb;
	...
	if (!fb)
		return -EINVAL;
	if (flags == FBLOCK_FLAGS_TO_HW){
		set_hw_flag(fb, flags);
	}
...
int fblock_userctl_unset_flags(struct lananlmsg *lmsg, unsigned int flags)
{
	struct fblock *fb;
	struct lananlmsg_flags *msg = (struct lananlmsg_flags *) lmsg->buff;
	fb = search_fblock_n(msg->name);
	...
	if (flags == FBLOCK_FLAGS_TO_HW){
		unset_hw_flag(fb, flags);
	}
	put_fblock(fb);
	return 0;
}

static int fblock_userctl_replace_hw(struct lananlmsg *lmsg)
{
	int ret;
	struct fblock *fb1, *fb2;
	struct lananlmsg_replace *msg =	(struct lananlmsg_replace *) lmsg->buff;
	reconfig_hw_block(msg->name2);
	return 0;
}
...
static int __fblock_userctl_rcv(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	case NETLINK_USERCTL_CMD_REPLACE:
		...
		ret = fblock_userctl_replace_hw(lmsg);
		break;
