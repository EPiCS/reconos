...
void setup_noc(void)
{
	...
	config_eth(0xcdcdcdcd, 0xcdcdcdcd, 2, TO_FB1); // HW - 1 // try
	config_eth(0xefefefef, 0xefefefef, 4, TO_FB2); // HW - 2
	config_eth(0xabcdabcd, 0xabcdabcd, 5, TO_FB3); // HW - 3
	// FBs
	mbox_put(&mb_in[HWT_FB1], TO_FB2);
	ret = mbox_get(&mb_out[HWT_FB1]);
	printf("FB1: %d\n", ret);
	mbox_put(&mb_in[HWT_FB2], TO_FB3);
	ret = mbox_get(&mb_out[HWT_FB2]);
	printf("FB2: %d\n", ret);
	mbox_put(&mb_in[HWT_FB3], TO_H2S);
	ret = mbox_get(&mb_out[HWT_FB3]);
	printf("FB3: %d\n", ret);
}
