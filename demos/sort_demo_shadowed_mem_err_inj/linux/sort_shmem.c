...
void *sort_shmem_thread(void* data)
{
    ...
    unsigned int * sort_buffer = malloc(2048*sizeof(unsigned int));
    while ( 1 ) {
		...
		start = gettime();
#endif
		memcpy(sort_buffer, (unsigned int *)address, length*sizeof(unsigned int));
		bubblesort( sort_buffer, length);
		memcpy((unsigned int *)address, sort_buffer, length*sizeof(unsigned int));
#ifdef BENCHMARK
		stop = gettime();
