...
-- This is a OSIF-less sort_demo for testing MEMIF implementations 
-- without the need to model OSIF interface and commands.
entity hwt_sort_demo_stub is
	port (
		-- FIFO Interface
...
architecture implementation of hwt_sort_demo_stub is
	type STATE_TYPE is (
...
begin
  ...
  begin
	...
	reconos_fsm: process (clk,rst,o_memif,o_ram) is
		...
		if rst = '1' then
			...
			case state_with_potential_fault is
                ...
                when STATE_THREAD_YIELD =>
                  ...
                  state <= STATE_GET_LEN;
                  temp <= X"00000800"; -- equals to 8kb of data
                  												   -- we have to set this a state earlier, so it is ready in the next state

        -- get length via mbox: the amount of data in words to be sorted, will be saved in a local register
				when STATE_GET_LEN =>
					temp <= X"100f3400"; -- a typical virtual address at which the sort data is stored
                  						 -- we have to set this a state earlier, so it is ready in the next state
                  	done:= true;
